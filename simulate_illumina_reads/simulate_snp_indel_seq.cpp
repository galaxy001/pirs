...
double snp_transition_by_transvertion_rate = 2; //transition_number ：transvertion_rate_number = 2	(比值)
int output_type = 1;
ofstream outfile;
...
void Usage(){
	...
	cout<<endl;
	cout<<"\tIt is a program for simulating heterozygosis in diploid, such as SNP, Indel and Invertion, ";
	cout<<"when simulate SNP, we can set the total SNP rate with option -s, and set the value of transition divided by transvertion with option -a. when simulate insertion and deletion, ";
	cout<<"we divide into two kinds of cases: 1.small SV(structural variation):insertion and deletion 1/2 of the total rate, 1~6bp bases as the indel number ,and the ";
	cout<<"rate distribution of each type as below: 1bp-64.82%, 2bp-17.17%, 3bp-7.20%, 4bp-7.29%, 5bp-2.18%, 6bp-1.34%, which refer to panda genome, we can ";
	cout<<"set the total small SV rate with option -d; 2.big SV: insertion, deletion and invertion, each of them is 1/3 of total rate, here we use this rate ";
	cout<<"distribution: 100bp-70%, 200bp-20%, 500bp-7%, 1000bp-2%, 2000bp-1%, we can set the total big SV rate with option -v; ";
	...
	cout<<"\t-d	<double>	set the small heterozygous indel rate of the diploid genome,default:"<<heterindel_rate<<endl;
	cout<<"\t-v	<double>	set the big structural variation rate(insertion,deletion,invertion) of the diploid genome,default:"<<big_SV_rate<<endl;
	...
	cout<<"\t1. ./simulate_snp_indel_seq -i ref_sequence.fa -s 0.001 -d 0.0001 -v 0.000001 -o ref_sequence >simulate_snp_indel.out 2>simulate_snp_indel.err"<<endl;
	exit(-1);
}
...
int main(int argc, char *argv[])
{
	...
	if(big_SV_rate < 0 || big_SV_rate > 1){cerr<<"Error: big_SV_rate should be set in 0~1 "<<endl;exit(-1);}
